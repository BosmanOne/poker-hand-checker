...
var handsToPrint = 3; //Sey number of hands to print
	...
	(function initiation() {
		for (var i=0; i<cardsSuits.length; i++){
			for (var j=0; j<cardsValues.length; j++){
				...
				var url = 'http://res.cloudinary.com/bosmanone/image/upload/v1477397924/cards/'+fileName +'.png';
...
$(document).on("mouseenter", ".hand-list-element", function() {
  ...
  for (i=0; i<hoverCardsArr.length; i++){
    $('.'+hoverCardsArr[i]).css({"box-shadow": "0px 0px 4px 2px red", "z-index": "2"});
  }
...
$(document).on("mouseleave", ".hand-list-element", function() {
  ...
  for (i=0; i<hoverCardsArr.length; i++){
    $('.'+hoverCardsArr[i]).css({"box-shadow": "", "z-index": "0"});
  }
